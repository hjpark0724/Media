...
)
{
    ...
    *ser_size = nb_of_bits[*mode];         move16();
    ...
    for (i = L_FRAME + L_FILT - 1; i > 0; i--)
    {
        ...
        new_speech[i] = rround(L_tmp);      move16();
    }

    L_tmp = L_mult(new_speech[0], 16384);
    L_tmp = L_msu(L_tmp, st->mem_preemph, mu);
    L_tmp = L_shl(L_tmp, Q_new);
    new_speech[0] = rround(L_tmp);          move16();
    ...
    Autocorr_Amr(p_window, M, r_h, r_l);       /* Autocorrelations */
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
        ...
        for (i = 0; i < L_SUBFR; i++)
        {
            ...
            h1[i] = error[i + M] = rround(L_shl(L_tmp, 3));      move16();move16();
        }
        ...
        gain_code = rround(L_tmp);          /* scaled gain_code with Qnew */
        ...
        for (i = 0; i < L_SUBFR; i++)
        {
            ...
            exc[i + i_subfr] = rround(L_tmp);    move16();
        }
        ...
        if (sub(*ser_size, NBBITS_24k) >= 0)
        {
            ...
            for (i = 1; i < L_SUBFR - 1; i++)
            {
                ...
                L_tmp = L_msu(L_tmp, code[i - 1], tmp);
                code2[i] = rround(L_tmp);   move16();
            }

            L_tmp = L_deposit_h(code[L_SUBFR - 1]);
            L_tmp = L_msu(L_tmp, code[L_SUBFR - 2], tmp);
            code2[L_SUBFR - 1] = rround(L_tmp);  move16();

            /* build excitation */

            gain_code = rround(L_shl(L_gain_code, Q_new));
