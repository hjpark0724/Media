 ...
 */
void autocorr(
     FFLOAT *x,              /* input : input signal x[0:L_WINDOW] */
     int m,                 /* input : LPC order                  */
     FFLOAT *r               /* output: auto-correlation vector r[0:M]*/
)
{
   static FFLOAT y[L_WINDOW];  /* dynamic memory allocation is used in real time*/
   FFLOAT sum;
   int i, j;
...
/*-------------------------------------------------------------*
 * procedure lag_window_729                                    *
 *           ~~~~~~~~~                                         *
 * lag windowing of the autocorrelations                       *
 *-------------------------------------------------------------*/

// Rename By Loche (Speex window.c lag_window duplicate name)

void lag_window_729(
     int m,                 /* input : LPC order                  */
     FFLOAT   r[]            /* in/out: correlation */
)
{
   int i;
 ...
 *----------------------------------------------------------------------------
 */
FFLOAT levinson(         /* output: prediction error (energy) */
 FFLOAT *r,              /* input : auto correlation coefficients r[0:M] */
 FFLOAT *a,              /* output: lpc coefficients a[0] = 1 */
 FFLOAT *rc              /* output: reflection coefficients rc[0:M-1]    */
)
{
   FFLOAT s, at, err;
   int i, j, l;
...
static FFLOAT chebyshev(FFLOAT x, FFLOAT *f, int n);

void az_lsp(
  FFLOAT *a,         /* input : LP filter coefficients                     */
  FFLOAT *lsp,       /* output: Line spectral pairs (in the cosine domain) */
  FFLOAT *old_lsp    /* input : LSP vector from past frame                 */
)
{
 int i, j, nf, ip;
 FFLOAT xlow,ylow,xhigh,yhigh,xmid,ymid,xint;
 FFLOAT *pf1;

 FFLOAT f1[NC+1], f2[NC+1];
...
FFLOAT chebyshev(   /* output: the value of the polynomial C(x)       */
  FFLOAT x,         /* input : value of evaluation; x=cos(freq)       */
  FFLOAT *f,        /* input : coefficients of sum or diff polynomial */
