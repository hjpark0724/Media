...
static FFLOAT dot_product(FFLOAT x[], FFLOAT y[], int lg);
...
int pitch_ol_fast(         /* output: open-loop pitch lag       */
    FFLOAT signal[],        /* input : signal to compute pitch   */
                           /*         s[-PIT_MAX : l_frame-1]   */
...
)
{
    ...
    *--------------------------------------------------------------------*/
    ...
    for(i=0; i<l_frame; i+=2, p+=2)
        sum += *p * *p;
    sum = (F)1.0/(FFLOAT)sqrt(sum);          /* 1/sqrt(energy)    */
...
static FFLOAT dot_product(   /* Return the dot product between x[] an y[] */
  FFLOAT x[],                /* First vector.                             */
  FFLOAT y[],                /* Second vector.                            */
...
FFLOAT g_pitch(          /* output: pitch gain                        */
  FFLOAT xn[],           /* input : target vector                     */
  FFLOAT y1[],           /* input : filtered adaptive codebook vector */
  FFLOAT g_coeff[],      /* output: <y1,y1> and -2<xn,y1>             */
