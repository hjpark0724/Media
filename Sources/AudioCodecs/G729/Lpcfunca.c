...
static void  get_lsp_pol(FFLOAT lsf[],FFLOAT f[]);
static void  lsp_az(FFLOAT *lsp, FFLOAT *a);
 ...
 */
static void lsp_az(
 FFLOAT *lsp,            /* input : lsp[0:M-1] */
 FFLOAT *a               /* output: predictor coeffs a[0:M], a[0] = 1. */
)
{
...
static void get_lsp_pol(
   FFLOAT lsp[],           /* input : line spectral freq. (cosine domain)  */
   FFLOAT f[]              /* output: the coefficients of F1 or F2 */
)
{
  FFLOAT b;
  int   i,j;
 ...
 */
void weight_az(
 FFLOAT *a,              /* input : lpc coefficients a[0:m]       */
 FFLOAT gamma,           /* input : weighting factor              */
 int m,                 /* input : filter order                  */
 FFLOAT *ap              /* output: weighted coefficients ap[0:m] */
)
{
  FFLOAT fac;
  int i;
 ...
 */
void int_qlpc(
 FFLOAT lsp_old[],       /* input : LSPs for past frame (0:M-1) */
 FFLOAT lsp_new[],       /* input : LSPs for present frame (0:M-1) */
 FFLOAT az[]             /* output: filter parameters in 2 subfr (dim 2(m+1)) */
)
{
  int i;
  FFLOAT lsp[M];
