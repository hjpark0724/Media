 ...
 *--------------------------------------------------------*/
 ...
 static FFLOAT old_speech[L_TOTAL];
 static FFLOAT *speech, *p_window;
 FFLOAT *new_speech;                    /* Global variable */

        /* Weighted speech vector */

 static FFLOAT old_wsp[L_FRAME+PIT_MAX];
 static FFLOAT *wsp;

        /* Excitation vector */

 static FFLOAT old_exc[L_FRAME+PIT_MAX+L_INTERPOL];
 static FFLOAT *exc;

        /* LSP Line spectral frequencies */

 static FFLOAT lsp_old[M] =
     { (F)0.9595,  (F)0.8413,  (F)0.6549,  (F)0.4154,  (F)0.1423,
      (F)-0.1423, (F)-0.4154, (F)-0.6549, (F)-0.8413, (F)-0.9595};

 static FFLOAT lsp_old_q[M];

        /* Filter's memory */

 static FFLOAT  mem_w0[M], mem_w[M], mem_zero[M];
 static FFLOAT  sharp;
...
)
{
   ...
   FFLOAT Aq_t[(MP1)*2];         /* A(z) quantized for the 2 subframes   */
   FFLOAT Ap_t[(MP1)*2];         /* A(z) with spectral expansion         */
   FFLOAT *Aq, *Ap;              /* Pointer on Aq_t  and Ap_t            */

   /* Other vectors */

   FFLOAT h1[L_SUBFR];           /* Impulse response h1[]              */
   FFLOAT xn[L_SUBFR];           /* Target vector for pitch search     */
   FFLOAT xn2[L_SUBFR];          /* Target vector for codebook search  */
   FFLOAT code[L_SUBFR];         /* Fixed codebook excitation          */
   FFLOAT y1[L_SUBFR];           /* Filtered adaptive excitation       */
   FFLOAT y2[L_SUBFR];           /* Filtered fixed codebook excitation */
   FFLOAT g_coeff[5];            /* Correlations between xn, y1, & y2:
 ...
 *------------------------------------------------------------------------*/
   {
      /* Temporary vectors */
     FFLOAT r[MP1];                    /* Autocorrelations       */
     FFLOAT rc[M];                     /* Reflexion coefficients */
     FFLOAT lsp_new[M];                /* lsp coefficients       */
     FFLOAT lsp_new_q[M];              /* Quantized lsp coeff.   */

     /* LP analysis */

     autocorr(p_window, M, r);             /* Autocorrelations */
     lag_window_729(M, r);                     /* Lag windowing    */
