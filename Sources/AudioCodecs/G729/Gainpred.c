 ...
 *---------------------------------------------------------------------------*/
void gain_predict(
   FFLOAT past_qua_en[], /* (i)     :Past quantized energies        */
   FFLOAT code[],        /* (i)     :Innovative vector.             */
   int l_subfr,         /* (i)     :Subframe length.               */
   FFLOAT *gcode0        /* (o)     :Predicted codebook gain        */
)
{
   FFLOAT ener_code, pred_code;
   int i;
   ...
   ener_code = (F)10.0 * (FFLOAT)log10(ener_code /(FFLOAT)l_subfr);
   ...
   *gcode0 = pred_code;
   *gcode0 = (FFLOAT)pow((double)10.0,(double)(*gcode0/20.0));   /* predicted gain */
...
void gain_update(
   ...
   FFLOAT g_code           /*  input: gbk1[indice1][1]+gbk2[indice2][1] */
)
{
   int i;
   ...
   past_qua_en[0] = (F)20.0*(FFLOAT)log10((double)g_code);
